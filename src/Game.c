...
void printGame(Game *g) {
  for (int i = 0; i < g->N; i++) {
    for (int j = 0; j < g->N; j++) {
      ...
      site.i = i;
      site.j = j;
      if (equalsSite(g->banditSite, g->policeSite) &&
          (equalsSite(g->banditSite, site)))
        printf("* ");
      else if (equalsSite(g->banditSite, site))
        printf("%c ", g->BANDIT);
      else if (equalsSite(g->policeSite, site))
        printf("%c ", g->POLICE);
      else if (isRoomScenario(g->scenario, site))
        printf(". ");
      else if (isCorridorScenario(g->scenario, site))
        printf("+ ");
      else if (isRoomScenario(g->scenario, site))
        printf(". ");
      else if (isWallScenario(g->scenario, site))
        printf("  ");
    }
    printf("\n");
  }
...
void playGame(Game *g) {
  if (g) {
    for (int t = 1; 1 == 1; t++) {
      ...
      if (isLegalMoveScenario(g->scenario, g->banditSite, next))
        g->banditSite = next;
      else {
      ...
      if (equalsSite(g->policeSite, g->banditSite))
        break;
      next = movePolice(g);
      if (isLegalMoveScenario(g->scenario, g->policeSite, next))
        g->policeSite = next;
      else {
...
int main(int argc, char *argv[]) {
  if (argc != 2) {
    printf("Way of use: c:>Proyecto3.exe scenario1.txt\n");
    return 1;
  }
